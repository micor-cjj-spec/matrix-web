<template>
  <div class="material-page">
    ...
    <v-dialog v-model="actionDialog.visible" max-width="420">
      <v-card>
        <v-card-title class="text-h6">物料操作</v-card-title>
        <v-card-text>
...
const { list, loading, fetchList, createItem, editItem, deleteItem } = useSimpleData(materialApi)
const { submitting, submitOne, submitBatch } = useReviewActions()

const headers = ref([
  { title: '名称', value: 'fname', align: 'start' },
  { title: '编码', value: 'fcode' },
  { title: '操作', value: 'actions', sortable: false, align: 'center', width: 180 },
])
