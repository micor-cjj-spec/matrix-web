  ...
  <div class="ledger-query-page">
    <v-card elevation="4" class="page-card">
      <div class="page-header">
        <div>
          <h2 class="page-title">核算维度余额表</h2>
          <div class="page-subtitle">第一期按现金流项目维度展示余额，后续再扩展部门、客户、供应商等维度。</div>
        </div>
...
const headers = [
  ...
  { title: '维度名称', key: 'dimensionName', value: 'dimensionName', minWidth: 200 },
...
const cards = computed(() => [
  { label: '维度数', value: String(summary.value.recordCount || 0), tone: 'tone-primary' },
  ...
  { label: '有发生维度', value: String(records.value.filter((item) => Number(item.entryCount || 0) > 0).length), tone: 'tone-success' },
])

async function fetchData() {
  loading.value = true
  try {
    const res = await ledgerApi.fetchDimensionBalance({
