  ...
  <div class="ledger-query-page">
    <v-card elevation="4" class="page-card">
      <div class="page-header">
        <div>
          <h2 class="page-title">辅助维度余额表</h2>
          <div class="page-subtitle">按现金流项目与科目组合展示辅助维度余额，是当前辅助核算第一期实现。</div>
        </div>
...
const cards = computed(() => [
  { label: '组合数', value: String(summary.value.recordCount || 0), tone: 'tone-primary' },
  ...
  { label: '有发生组合', value: String(records.value.filter((item) => Number(item.entryCount || 0) > 0).length), tone: 'tone-success' },
])

async function fetchData() {
  loading.value = true
  try {
    const res = await ledgerApi.fetchAuxDimensionBalance({
