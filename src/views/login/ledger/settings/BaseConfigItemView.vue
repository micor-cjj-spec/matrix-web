  ...
  <div class="settings-page">
    <v-card elevation="4" class="pa-6">
      ...
      <v-row dense class="mb-4">
        ...
        <v-col cols="12" md="2">
          <v-text-field v-model.trim="query.fcode" label="编码" density="comfortable" clearable />
        </v-col>
        <v-col cols="12" md="3">
          <v-text-field v-model.trim="query.fname" label="名称" density="comfortable" clearable />
        </v-col>
        <v-col cols="12" md="2">
          <v-select v-model="query.fstatus" :items="statusOptions" item-title="label" item-value="value" label="状态" density="comfortable" clearable />
        </v-col>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-row dense>
            ...
            <v-col cols="12" md="6">
              <v-text-field v-model.trim="form.fvalue" :label="pageMeta.valueLabel" density="comfortable" />
            </v-col>
            <v-col cols="12" md="6">
...
import baseConfigItemApi from '@/api/baseConfigItem'
...
const pageConfigs = {
  dimensionRelation: {
    title: '核算维度关系设置',
    subtitle: '维护核算维度之间的关系定义，例如科目与部门、客户、供应商之间的适配关系。',
    valueLabel: '关系表达式',
    category: 'DIMENSION_RELATION',
  },
  dimensionValueRange: {
    title: '核算维度值范围设置',
    subtitle: '维护维度可用值范围或取值规则，为凭证录入和维度账查询提供约束。',
    valueLabel: '范围定义',
    category: 'DIMENSION_VALUE_RANGE',
  },
  equityChangeType: {
    title: '所有者权益变动类型',
    subtitle: '维护权益变动项目分类，供权益报表和相关科目映射使用。',
    valueLabel: '变动说明',
    category: 'EQUITY_CHANGE_TYPE',
  },
  impairmentNature: {
    title: '减值准备性质',
    subtitle: '维护减值准备相关性质定义，便于报表披露和科目分类。',
    valueLabel: '性质说明',
    category: 'IMPAIRMENT_NATURE',
  },
  licensePlateItem: {
    title: '车辆牌照号项目',
    subtitle: '维护车辆牌照号相关项目，用于固定资产、费用和共享运营场景扩展。',
    valueLabel: '牌照规则',
    category: 'LICENSE_PLATE_ITEM',
  },
  costNature: {
    title: '成本性质',
    subtitle: '维护成本性质分类，为成本归集和分析报表提供统一口径。',
    valueLabel: '性质说明',
    category: 'COST_NATURE',
  },
}

const pageMeta = computed(() => pageConfigs[route.meta?.baseConfigModule] || pageConfigs.dimensionRelation)
...
const headers = [
  ...
  { title: '编码', key: 'fcode', value: 'fcode', width: 140 },
  { title: '名称', key: 'fname', value: 'fname' },
  { title: '配置值', key: 'fvalue', value: 'fvalue' },
...
function emptyForm() {
  return {
    fid: null,
    forg: null,
    fcategory: pageMeta.value.category,
    fcode: '',
    fname: '',
    fvalue: '',
    fstatus: 'ENABLED',
...
async function fetchData() {
  loading.value = true
  try {
    const res = await baseConfigItemApi.fetchList({
      page: query.page,
      size: query.size,
...
async function handleSave() {
  try {
    ...
    payload.fcategory = pageMeta.value.category
    if (dialog.mode === 'create') {
      await baseConfigItemApi.createItem(payload)
      showMsg(`${pageMeta.value.title}新增成功`, 'success')
    } else {
      await baseConfigItemApi.editItem(payload)
...
async function handleDelete(item) {
  try {
    await baseConfigItemApi.deleteItem(item.fid)
